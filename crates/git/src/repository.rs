...
impl GitRepository for RealGitRepository {
    ...
    fn commit(
        ...
        async move {
            let git = git_binary?;
            let message_file_path = repository
                .lock()
                .path()
                .join(format!("ZED_COMMIT_EDITMSG_{}", Uuid::new_v4()));
...
async fn commit_msg_hook_path(
    ...
    repository.lock().path().join("hooks").join("commit-msg")
}
...
mod tests {
    ...
    async fn test_commit_signoff_is_applied_before_commit_msg_hook(cx: &mut TestAppContext) {
        ...
        let repo_dir = tempfile::tempdir().unwrap();
        let repository = Arc::new(Mutex::new(git2::Repository::init(repo_dir.path()).unwrap()));
        let hook_path = repo_dir.path().join(".git/hooks/commit-msg");
        smol::fs::write(
            &hook_path,
        ...
        apply_commit_signoff(&git, &message_file_path, &env)
            .await
            .unwrap();
        run_commit_msg_hook(&git, repository, "".into(), &message_file_path, &env)
            .await
            .unwrap();
