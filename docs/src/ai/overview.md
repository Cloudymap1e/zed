...
| ------------ | ----------------------------------- | -------------------------------------------------------------------- |
| Agents       | How agentic work runs in Zed        | External Agents, Terminal Threads                                    |
...
- [External Agents](./external-agents.md): agents that run through their own process and configuration while Zed hosts the thread UI.
...
Model access controls which models power Zed AI features such as Inline Assistant, Git commit generation, thread summaries, and similar workflows. Zed can use hosted models, provider API access, subscription sign-in, gateways, and local models.
