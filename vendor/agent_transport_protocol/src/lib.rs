...
//! # agent-transport-protocol -- the agent transport protocol SDK
//!
//! **agent-transport-protocol** is a Rust SDK for connecting AI agents and clients
//! with tool use, permission requests, and streaming responses.
//!
//! ## What can you build with agent-transport-protocol?
//!
//! - **Clients** that talk to external agents (like building your own Claude Code interface)
...
//!
//! The most common use case is connecting to an existing external agent as a client.
//! Here's a minimal example that initializes a connection, creates a session,
//! and sends a prompt:
//!
//! ```no_run
//! use agent_transport_protocol::Client;
//! use agent_transport_protocol::schema::{ProtocolVersion, v1::InitializeRequest};
//!
//! # async fn run(transport: impl agent_transport_protocol::ConnectTo<agent_transport_protocol::Client>) -> agent_transport_protocol::Result<()> {
//! Client.builder()
//!     .name("my-client")
...
//!
//! [yolo]: examples/yolo_one_shot_client.rs
//!
//! ## Cookbook
//!
//! The [`agent_transport_protocol_cookbook`] crate contains practical guides and examples:
//!
...
//!
//! [`agent_transport_protocol_cookbook`]: https://docs.rs/agent-transport-protocol-cookbook
//!
//! ## Core Concepts
//!
//! The [`concepts`] module provides detailed explanations of how agent-transport-protocol works,
//! including connections, sessions, callbacks, ordering guarantees, and more.
//!
//! ## Related Crates
//!
//! - [`agent-transport-protocol-conductor`] - Binary for running proxy chains
//!
//! [`agent-transport-protocol-conductor`]: https://crates.io/crates/agent-transport-protocol-conductor

/// Capability management for the `_meta.symposium` object
mod capabilities;
/// Component abstraction for agents and proxies
pub mod component;
/// Core concepts for understanding and using agent-transport-protocol
pub mod concepts;
/// JSON-RPC connection and handler infrastructure
mod jsonrpc;
/// MCP server support for providing MCP tools over agent sessions
pub mod mcp_server;
/// Role types for agent transport connections
pub mod role;
/// Agent transport schema types - all message types, requests, responses, and supporting types
pub mod schema;
...
pub use role::{
    ...
    agent_transport::{Agent, Client, Conductor, Proxy},
};
...
pub use agent_transport_protocol_derive::{JsonRpcNotification, JsonRpcRequest, JsonRpcResponse};

mod session;
pub use session::*;

mod external_agent_process;
pub use external_agent_process::{ExternalAgentProcess, LineDirection};
...
///
/// The `agent-transport-protocol-rmcp` crate provides the builder this macro is
/// typically used with.
