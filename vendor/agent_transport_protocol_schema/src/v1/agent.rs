...
pub struct NewSessionResponse {
    ...
    ///
    /// See protocol docs: [Session Modes](https://agent-transport-protocol/protocol/session-modes)
    #[serde_as(deserialize_as = "DefaultOnError")]
    #[schemars(extend("x-deserialize-default-on-error" = true))]
    #[serde(default)]
...
impl SetSessionModeRequest {
    ...
    /// Sets or clears agent transport `_meta` extension metadata.
    #[must_use]
    pub fn meta(mut self, meta: impl IntoOption<Meta>) -> Self {
        self.meta = meta.into_option();
        self
...
pub enum McpServer {
    ...
    ///
    /// Only available when the Agent capabilities indicate `mcp_capabilities.agent_transport` is `true`.
...
///
/// See protocol docs: [User Message](https://agent-transport-protocol/protocol/prompt-turn#1-user-message)
#[skip_serializing_none]
...
///
/// See protocol docs: [Check for Completion](https://agent-transport-protocol/protocol/prompt-turn#4-check-for-completion)
#[serde_as]
#[skip_serializing_none]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
#[schemars(extend("x-side" = "agent", "x-method" = SESSION_PROMPT_METHOD_NAME))]
...
///
/// See protocol docs: [Stop Reasons](https://agent-transport-protocol/protocol/prompt-turn#stop-reasons)
...
impl McpCapabilities {
    ...
    #[cfg(feature = "unstable_mcp_over_agent_transport")]
    #[must_use]
    pub fn agent_transport(mut self, agent_transport: bool) -> Self {
...
pub enum ClientRequest {
    ...
    ///
    /// See protocol docs: [Prompt Turn](https://agent-transport-protocol/protocol/prompt-turn)
...
pub enum ClientNotification {
    ...
    ///
    /// See protocol docs: [Cancellation](https://agent-transport-protocol/protocol/prompt-turn#cancellation)
...
mod test_serialization {
    ...
    #[cfg(feature = "unstable_mcp_over_agent_transport")]
    #[test]
    fn test_mcp_server_agent_transport_serialization() {
        let server = McpServer::AgentTransport(McpServerAgentTransport::new(
            "project-tools",
            "project-tools-id",
        ));
        ...
        let deserialized: McpServer = serde_json::from_value(json).unwrap();
        match deserialized {
            McpServer::AgentTransport(McpServerAgentTransport { name, id, meta: _ }) => {
                assert_eq!(name, "project-tools");
                assert_eq!(id, McpServerAgentTransportId::new("project-tools-id"));
            }
            _ => panic!("Expected AgentTransport variant"),
        }
    }
