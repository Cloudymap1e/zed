...
description: Install and use External Agents such as Claude, Codex, OpenCode, Copilot, Cursor, and Pi Coding Agent in Zed.
---

# External Agents

External Agents are agents that integrate with Zed through the External Agent protocol. Zed hosts the thread in the [Agent Panel](./agent-panel.md) and [Threads Sidebar](./parallel-agents.md#threads-sidebar), while the External Agent usually owns its own runtime, auth, model selection, tools, and native configuration.
...
## Install from the Agent Registry {#registry}

The Agent Registry is the primary way to install common External Agents in Zed.

Open the registry with {#action zed::AgentRegistry}, or open [Agent Settings](./agent-settings.md) with {#action agent::OpenSettings}, go to the **External Agents** page, click `Add Agent`, and choose `Install from Registry`.
...
This list is curated, not exhaustive. Open the Agent Registry in Zed for the current list of available agents.
...
Use Claude Agent when you want Claude running as an External Agent in Zed.

Install Claude Agent from the [Agent Registry](#registry), then start a Claude Agent thread from the Agent Panel or Threads Sidebar. Claude Agent owns its own authentication and billing. An Anthropic API key configured in Zed LLM Providers does not automatically configure Claude Agent.
...
Use Codex when you want Codex running as an External Agent in Zed.

Install Codex from the [Agent Registry](#registry), then start a Codex thread from the Agent Panel or Threads Sidebar. Codex owns its own authentication and billing. An OpenAI API key configured in Zed LLM Providers does not automatically configure Codex.
...
Use Gemini CLI when you want Gemini running as an External Agent in Zed.

Install Gemini CLI from the [Agent Registry](#registry), then start a Gemini CLI thread from the Agent Panel or Threads Sidebar. Gemini CLI owns its own authentication and may prompt you to log in with Google, Vertex AI, or another Gemini-supported flow.
...
Use OpenCode when you want OpenCode running as an External Agent in Zed.

Install OpenCode from the [Agent Registry](#registry), then start an OpenCode thread from the Agent Panel or Threads Sidebar. OpenCode owns its own auth, model selection, and subscription behavior. To use OpenCode models in Zed AI features, configure [OpenCode API access](./use-api-access.md#opencode).

## Copilot {#copilot}

Use Copilot External Agents where available when you want Copilot running as an External Agent in Zed.

Copilot agent auth is owned by the Copilot integration. To use Copilot Chat models in Zed AI features or Copilot for edit prediction, see [Use an Existing Subscription](./use-an-existing-subscription.md#github-copilot).

## Cursor {#cursor}

Use Cursor External Agents where available when you want Cursor running as an External Agent in Zed.
...
Use Pi Coding Agent when you want Pi running as an External Agent in Zed.
...
External Agents run as separate processes that communicate with Zed through the External Agent protocol. This creates a boundary between Zed configuration and agent-native configuration.
...
| Zed-managed profiles             | Do not apply unless the integration says otherwise                                         |
| Zed Skills                       | Do not apply as Zed Skills                                                                 |
| Native agent skills/instructions | Depends on the agent                                                                       |
| Zed MCP servers                  | May be forwarded through the External Agent protocol                                        |
| Native MCP config                | May also be read by the agent                                                              |
| Tool permissions                 | Zed External Agent tool forwarding permissions may apply; native tool permissions depend on the agent |
...
Use custom agents when you are developing an External Agent-compatible agent or need to run an agent that is not in the registry.
...
Extension-provided agents are deprecated. The [Agent Registry](#registry) is now the way to install agents, and previously installed extension agents are automatically migrated to their registry equivalents.
...
Zed connects to each selected agent through the External Agent protocol and adds sessions that are not already in your history. Imported threads are archived entries; open one to restore it and continue where you left off.
...
Zed-configured [MCP servers](./mcp.md) may be forwarded to External Agents through the External Agent protocol. External Agents may also read their own native MCP configuration.
