...
const ZED_SESSION_TOKEN_USAGE_META_KEY: &str = "zed.private.token_usage";
...
pub trait CodexThreadImpl: Send + Sync {
    ...
    fn goals_available(&self) -> bool;
    fn get_thread_goal(
        &self,
    ) -> Pin<Box<dyn Future<Output = anyhow::Result<Option<ThreadGoal>>> + Send + '_>>;
    fn set_thread_goal(
        &self,
        objective: Option<String>,
        status: Option<ThreadGoalStatus>,
    ) -> Pin<Box<dyn Future<Output = anyhow::Result<ThreadGoal>> + Send + '_>>;
    fn clear_thread_goal(&self) -> Pin<Box<dyn Future<Output = anyhow::Result<bool>> + Send + '_>>;
}
...
impl CodexThreadImpl for CodexThread {
    ...
    fn goals_available(&self) -> bool {
        self.enabled(Feature::Goals) && self.state_db().is_some()
    }
    ...
    fn get_thread_goal(
        ...
        Box::pin(async move {
            ...
            let state_db = self
                .state_db()
                .ok_or_else(|| anyhow::anyhow!("sqlite state db unavailable for thread goals"))?;
            let thread_id = self.session_configured().thread_id;
            state_db
                .get_thread_goal(thread_id)
                .await
                .map(|goal| goal.map(protocol_thread_goal_from_state))
        })
    }
    ...
    fn set_thread_goal(
        ...
        Box::pin(async move {
            ...
            let (goal, previous_status) = if let Some(objective) = objective.as_deref() {
                ...
                let Some(existing_goal) = existing_goal else {
                    anyhow::bail!("cannot update goal for thread {thread_id}: no goal exists");
                };
            ...
            self.apply_external_goal_set(external_goal_set).await;
            Ok(protocol_thread_goal_from_state(goal))
        })
    }
    ...
    fn clear_thread_goal(&self) -> Pin<Box<dyn Future<Output = anyhow::Result<bool>> + Send + '_>> {
        Box::pin(async move {
            ...
            let thread_id = self.session_configured().thread_id;
            self.prepare_external_goal_mutation().await;
            let cleared = state_db.delete_thread_goal(thread_id).await?;
            if cleared {
...
impl SubmissionState {
    ...
    fn adopt_submission_id(&mut self, submission_id: String) {
        match self {
            Self::Prompt(state) => {
        ...
        match event {
            ...
            EventMsg::TokenCount(TokenCountEvent { info, .. }) => {
                    ...
                    && let Some(size) = info.model_context_window
                {
                    ...
                    let input_tokens = total_usage.non_cached_input().max(0) as u64;
                    let output_tokens = total_usage.output_tokens.max(0) as u64;
                    let cache_read_input_tokens = total_usage.cached_input().max(0) as u64;
                    let cache_creation_input_tokens = 0_u64;
                    let used = (total_usage.tokens_in_context_window().max(0) as u64).max(
                        input_tokens
                    ...
                    let meta = Meta::from_iter([(
                        ZED_SESSION_TOKEN_USAGE_META_KEY.into(),
                        json!({
...
struct ThreadActor<A> {
    ...
    /// A `/goal` command that may be adopted by the continuation turn Codex starts internally.
    pending_goal_submission: Option<SubmissionState>,
...
impl<A: Auth> ThreadActor<A> {
    ...
    async fn spawn(mut self) {
        ...
        loop {
            ...
            if !message_rx_open
                ...
                && self.pending_goal_submission.is_none()
            {
                break;
            }
        }
    ...
    async fn handle_message(&mut self, message: ThreadMessage) {
        match message {
            ThreadMessage::Load { response_tx } => {
                ...
                // in a separate task
                let include_goal_command = self.thread.goals_available();
                tokio::spawn(async move {
                    tokio::time::sleep(Duration::from_millis(200)).await;
                    client.send_notification(SessionUpdate::AvailableCommandsUpdate(
                        AvailableCommandsUpdate::new(Self::builtin_commands(include_goal_command)),
                    ));
                });
            }
            ...
            ThreadMessage::GoalCommandAdoptionTimeout {
                pending_submission_id,
            } => {
                self.complete_unadopted_goal_submission(&pending_submission_id);
            }
            #[cfg(test)]
            ThreadMessage::CodexEvent(event) => {
    ...
    fn builtin_commands(include_goal_command: bool) -> Vec<AvailableCommand> {
        ...
        if include_goal_command {
            commands.push(
                AvailableCommand::new("goal", "Set, show, pause, resume, or clear the thread goal")
                    .input(AvailableCommandInput::Unstructured(
    ...
    fn finish_goal_command(
        &self,
        response_tx: oneshot::Sender<Result<StopReason, Error>>,
        message: impl Into<String>,
    ) {
        self.client.send_agent_text(message.into());
        Self::finish_prompt_response(response_tx, Ok(StopReason::EndTurn));
    }
    ...
    fn format_goal_summary(goal: &ThreadGoal) -> String {
        let mut lines = vec![
            format!("Goal ({})", Self::goal_status_label(goal.status)),
            format!("Objective: {}", goal.objective.trim()),
            format!("Time used: {}s", goal.time_used_seconds),
    ...
    fn format_thread_goal_update_from_goal(goal: ThreadGoal) -> String {
        format_thread_goal_update(&ThreadGoalUpdatedEvent {
            thread_id: goal.thread_id,
            turn_id: None,
            goal,
        })
    ...
    fn complete_unadopted_goal_submission(&mut self, pending_submission_id: &str) {
        let should_complete = self
            .pending_goal_submission
            .as_ref()
            .is_some_and(|submission| match submission {
                SubmissionState::Prompt(state) => state.submission_id == pending_submission_id,
            });
        if !should_complete {
            return;
        }
        if let Some(mut submission) = self.pending_goal_submission.take() {
            match &mut submission {
                SubmissionState::Prompt(state) => {
    ...
    fn wait_for_goal_continuation(
        &mut self,
        response_tx: oneshot::Sender<Result<StopReason, Error>>,
    ) {
        let pending_submission_id = format!("goal-{}", Uuid::new_v4());
        self.pending_goal_submission = Some(SubmissionState::Prompt(PromptState::new(
            pending_submission_id.clone(),
            self.thread.clone(),
        ...
        tokio::spawn(async move {
            ...
            drop(
                resolution_tx.send(ThreadMessage::GoalCommandAdoptionTimeout {
                    pending_submission_id,
                }),
            );
        });
    ...
    async fn handle_goal_command(
        ...
        if !self.thread.goals_available() {
            self.finish_goal_command(
                response_tx,
                "Thread goals are unavailable for this session.",
            );
            return;
        }
        ...
        if trimmed.is_empty() || trimmed.eq_ignore_ascii_case("status") {
            match self.thread.get_thread_goal().await {
                Ok(Some(goal)) => {
                    self.finish_goal_command(response_tx, Self::format_goal_summary(&goal))
                }
                Ok(None) => self
                    .finish_goal_command(response_tx, "No goal is set.\nUsage: /goal <objective>"),
                Err(err) => self
                    .finish_goal_command(response_tx, format!("Failed to read thread goal: {err}")),
            }
            return;
        }

        match trimmed.to_ascii_lowercase().as_str() {
            "clear" => match self.thread.clear_thread_goal().await {
                Ok(true) => self.finish_goal_command(response_tx, "Goal cleared."),
                Ok(false) => self.finish_goal_command(response_tx, "No goal is set."),
                Err(err) => self.finish_goal_command(
                    response_tx,
            ...
            "pause" => match self
                .thread
                .set_thread_goal(None, Some(ThreadGoalStatus::Paused))
                .await
            {
                Ok(goal) => self.finish_goal_command(
                    response_tx,
                    Self::format_thread_goal_update_from_goal(goal),
                ),
                Err(err) => self.finish_goal_command(
                    response_tx,
                    format!("Failed to pause thread goal: {err}"),
                ),
            },
            "resume" => match self
                .thread
                .set_thread_goal(None, Some(ThreadGoalStatus::Active))
                .await
            {
                Ok(goal) => {
                    self.client
                        .send_agent_text(Self::format_thread_goal_update_from_goal(goal));
                    self.wait_for_goal_continuation(response_tx);
                }
                Err(err) => self.finish_goal_command(
                    response_tx,
                    format!("Failed to resume thread goal: {err}"),
                ),
            },
            _ => {
                if let Err(err) = validate_thread_goal_objective(trimmed) {
                    self.finish_goal_command(response_tx, err);
                    return;
                }
                match self
                    .thread
                    .set_thread_goal(Some(trimmed.to_owned()), Some(ThreadGoalStatus::Active))
                    .await
                {
                    Ok(goal) => {
                        self.client
    ...
    async fn handle_prompt(
        ...
        if let Some((name, rest)) = extract_slash_command(&items) {
            match name {
                ...
                "goal" => {
                    self.handle_goal_command(rest, response_tx).await;
    ...
    async fn handle_event(&mut self, Event { id, msg }: Event) {
        ...
        } else if let Some(mut submission) = self.pending_goal_submission.take() {
            submission.adopt_submission_id(id.clone());
            submission.handle_event(&self.client, msg).await;
...
mod tests {
    ...
    #[tokio::test]
    async fn test_token_count_usage_update_uses_cumulative_total() -> anyhow::Result<()> {
        let (session_id, client, _conversation, message_tx, _handle) = setup().await?;
        ...
        message_tx.send(ThreadMessage::CodexEvent(Event {
            id: "0".to_string(),
        ...
        let usage_update = notifications
            .iter()
            .find_map(|notification| match &notification.update {
                SessionUpdate::UsageUpdate(update) => Some(update),
                _ => None,
            })
            .expect("usage update should be sent");
        assert_eq!(usage_update.used, 1_500);
        assert_eq!(usage_update.size, 10_000);
        assert_eq!(usage_update.cost, None);
        let token_usage = usage_update
            .meta
            .as_ref()
            .and_then(|meta| meta.get(ZED_SESSION_TOKEN_USAGE_META_KEY))
            .expect("private token usage meta should be sent");
        assert_eq!(token_usage["input_tokens"], 750);
        assert_eq!(token_usage["output_tokens"], 500);
        assert_eq!(token_usage["cache_read_input_tokens"], 250);
    ...
    #[tokio::test]
    async fn test_goal_sets_thread_goal_without_user_input() -> anyhow::Result<()> {
        let (session_id, client, thread, message_tx, _handle) = setup().await?;
        let (prompt_response_tx, prompt_response_rx) = tokio::sync::oneshot::channel();

        message_tx.send(ThreadMessage::Prompt {
            request: PromptRequest::new(session_id.clone(), vec!["/goal ship it".into()]),
            response_tx: prompt_response_tx,
        })?;
        ...
        assert!(notifications.iter().any(|notification| {
            matches!(
                ...
                SessionUpdate::AgentMessageChunk(ContentChunk {
                    content: ContentBlock::Text(TextContent { text, .. }),
                    ..
                }) if text == "Goal updated (active): ship it"
            )
        }));
    ...
    #[tokio::test]
    async fn test_goal_pause_resume_and_clear() -> anyhow::Result<()> {
        let (session_id, client, thread, message_tx, _handle) = setup().await?;
        *thread.goal.lock().unwrap() = Some(ThreadGoal {
        ...
        for prompt in ["/goal pause", "/goal resume", "/goal clear"] {
            let (prompt_response_tx, prompt_response_rx) = tokio::sync::oneshot::channel();
            message_tx.send(ThreadMessage::Prompt {
        ...
        assert!(thread.goal.lock().unwrap().is_none());
        let notifications = client.notifications.lock().unwrap();
        assert!(notifications.iter().any(|notification| {
            matches!(
                &notification.update,
                SessionUpdate::AgentMessageChunk(ContentChunk {
                    content: ContentBlock::Text(TextContent { text, .. }),
                    ..
                }) if text == "Goal updated (paused): Keep shipping"
            )
        }));
        assert!(notifications.iter().any(|notification| {
            matches!(
    ...
    struct StubCodexThread {
        ...
        goal: std::sync::Mutex<Option<ThreadGoal>>,
    ...
    impl CodexThreadImpl for StubCodexThread {
        ...
        fn get_thread_goal(
            ...
            Box::pin(async { Ok(self.goal.lock().unwrap().clone()) })
        }
        ...
        fn set_thread_goal(
            ...
            Box::pin(async move {
                ...
                let updated_goal = if let Some(objective) = objective {
                    ThreadGoal {
                        thread_id: ThreadId::default(),
                        objective,
                        status,
                    ...
                    let mut updated_goal = goal
                        .clone()
                        .ok_or_else(|| anyhow::anyhow!("cannot update goal: no goal exists"))?;
                    updated_goal.status = status;
                    updated_goal.updated_at = updated_goal.updated_at.saturating_add(1);
                    updated_goal
                };
                *goal = Some(updated_goal.clone());
                if status == ThreadGoalStatus::Active {
                    let id = self
                        .current_id
                        .fetch_add(1, std::sync::atomic::Ordering::SeqCst)
                        .to_string();
                    self.op_tx.send(Event {
                        id: id.clone(),
                        msg: EventMsg::TurnComplete(TurnCompleteEvent {
                            last_agent_message: None,
                            turn_id: id,
        ...
        fn clear_thread_goal(
            ...
            Box::pin(async { Ok(self.goal.lock().unwrap().take().is_some()) })
        }
    }
