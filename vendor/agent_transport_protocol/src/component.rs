...
//! that can be run as part of a conductor's chain - agents, proxies, or any agent-transport component.
//!
//! ## Usage
//!
...
//!
//! ```rust,ignore
//! use agent_transport_protocol::{Agent, Client, Connect, Result};
//!
//! struct MyAgent {
...
/// (e.g., an agent [`Agent`](`crate::role::agent_transport::Agent`) or an MCP [`Server`](`crate::role::mcp::Server`)).
///
/// This trait represents anything that can communicate via JSON-RPC messages over channels -
/// agents, proxies, in-process connections, or any agent-transport component.
///
...
/// - **[`ExternalAgentProcess`]**: An external agent running in a separate process with stdio communication
/// - **Custom components**: Proxies, transformers, or any agent-transport-aware service
///
/// # Two Ways to Serve
///
...
///
/// ```rust,ignore
/// use agent_transport_protocol::{Agent, Result, Serve, role::Client};
///
/// struct MyAgent {
///     config: AgentConfig,
/// }
...
/// [`ExternalAgentProcess`]: crate::ExternalAgentProcess
...
///
/// ```rust,ignore
/// use agent_transport_protocol::{DynConnectTo, Client};
///
/// let components: Vec<DynConnectTo<Client>> = vec![
///     DynConnectTo::new(Proxy1),
///     DynConnectTo::new(Proxy2),
