//! Protocol types for proxy and MCP-over-agent-transport communication.
//!
//! These types are intended to become part of the agent transport protocol specification.

use crate::{JsonRpcMessage, JsonRpcNotification, JsonRpcRequest, UntypedMessage};
use agent_transport_protocol_schema::v1::{InitializeRequest, InitializeResponse};
...
// =============================================================================
// MCP-over-agent-transport protocol
// =============================================================================
...
pub struct McpConnectRequest {
    /// The agent transport identifier for the server.
    #[serde(rename = "agent_transport_id")]
    pub agent_transport_id: String,
...
/// JSON-RPC method name for MCP requests over agent transport
pub const METHOD_MCP_MESSAGE: &str = "_mcp/message";

/// An MCP request sent via agent transport. This could be an MCP-server-to-MCP-client request
/// (in which case it goes from the client to the agent,
/// note the reversal of roles) or an MCP-client-to-MCP-server request
/// (in which case it goes from the agent to the client).
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct McpOverAgentTransportMessage<M = UntypedMessage> {
...
impl<M: JsonRpcMessage> JsonRpcMessage for McpOverAgentTransportMessage<M> {
    fn matches_method(method: &str) -> bool {
    ...
    fn parse_message(method: &str, params: &impl Serialize) -> Result<Self, crate::Error> {
        ...
        let outer = crate::util::json_cast_params::<_, McpOverAgentTransportMessage<UntypedMessage>>(
            params,
        )?;
        if !M::matches_method(&outer.message.method) {
            return Err(crate::Error::method_not_found());
        ...
        let inner = M::parse_message(&outer.message.method, &outer.message.params)?;
        Ok(McpOverAgentTransportMessage {
...
impl<R: JsonRpcRequest> JsonRpcRequest for McpOverAgentTransportMessage<R> {
    type Response = R::Response;
}

impl<R: JsonRpcNotification> JsonRpcNotification for McpOverAgentTransportMessage<R> {}
