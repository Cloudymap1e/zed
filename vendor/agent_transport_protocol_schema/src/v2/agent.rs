...
//! an AI coding agent that follows the agent transport protocol (agent transport).
...
///
/// See protocol docs: [Initialization](https://agent-transport-protocol/protocol/initialization)
#[serde_as]
...
pub enum AuthMethod {
    ...
    /// future agent transport variants.
    ///
...
///
/// See protocol docs: [Creating a Session](https://agent-transport-protocol/protocol/session-setup#creating-a-session)
#[skip_serializing_none]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
...
///
/// See protocol docs: [Loading Sessions](https://agent-transport-protocol/protocol/session-setup#loading-sessions)
#[skip_serializing_none]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
...
///
/// Category names beginning with `_` are free for custom use, like other agent transport extension methods.
/// Category names that do not begin with `_` are reserved for the agent transport spec.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
...
/// processing prompts.
///
/// See protocol docs: [MCP Servers](https://agent-transport-protocol/protocol/session-setup#mcp-servers)
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
#[serde(tag = "type", rename_all = "snake_case")]
#[schemars(extend("discriminator" = {"propertyName": "type"}))]
#[non_exhaustive]
pub enum McpServer {
    ...
    ///
    /// Only available when the Agent capabilities include `session.mcp.agent_transport`.
    /// The MCP server is provided by an agent transport component and communicates over the agent transport channel.
    #[cfg(feature = "unstable_mcp_over_agent_transport")]
    #[serde(rename = "agent_transport")]
    AgentTransport(McpServerAgentTransport),
...
/// This capability is not part of the spec yet, and may be removed or changed at any point.
///
/// Unique identifier for an MCP server using the agent transport.
///
/// The value is opaque and generated by the agent transport component providing the MCP server. It is
...
#[from(Arc<str>, String, &'static str)]
#[non_exhaustive]
pub struct McpServerAgentTransportId(pub Arc<str>);

#[cfg(feature = "unstable_mcp_over_agent_transport")]
impl McpServerAgentTransportId {
    /// Wraps a protocol string as a typed [`McpServerAgentTransportId`].
    #[must_use]
    pub fn new(id: impl Into<Arc<str>>) -> Self {
        Self(id.into())
    }
...
/// This capability is not part of the spec yet, and may be removed or changed at any point.
///
/// agent transport configuration for MCP.
///
/// The MCP server is provided by an agent transport component and communicates over the agent transport channel
...
#[serde(rename_all = "camelCase")]
#[non_exhaustive]
pub struct McpServerAgentTransport {
    /// Human-readable name identifying this MCP server.
    pub name: String,
    /// Unique identifier for this MCP server, generated by the component providing it.
    ///
    /// Providers MUST NOT reuse an ID for multiple agent transport-transport MCP servers that are visible
    /// on the same agent transport connection.
    pub id: McpServerAgentTransportId,
    /// The _meta property is reserved by agent transport to allow clients and agents to attach additional
    /// metadata to their interactions. Implementations MUST NOT make assumptions about values at
    /// these keys.
    ///
...
#[cfg(feature = "unstable_mcp_over_agent_transport")]
impl McpServerAgentTransport {
    /// Builds [`McpServerAgentTransport`] with the required fields set; optional fields start unset or empty.
    #[must_use]
    pub fn new(name: impl Into<String>, id: impl Into<McpServerAgentTransportId>) -> Self {
        Self {
            name: name.into(),
            id: id.into(),
            meta: None,
...
///
/// See protocol docs: [User Message](https://agent-transport-protocol/protocol/prompt-lifecycle#1-user-message)
#[skip_serializing_none]
...
///
/// See protocol docs: [Prompt Accepted](https://agent-transport-protocol/protocol/prompt-lifecycle#2-prompt-accepted)
#[skip_serializing_none]
#[derive(Default, Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
#[schemars(extend("x-side" = "agent", "x-method" = SESSION_PROMPT_METHOD_NAME))]
#[serde(rename_all = "camelCase")]
...
///
/// See protocol docs: [Stop Reasons](https://agent-transport-protocol/protocol/prompt-lifecycle#stop-reasons)
...
///
/// Protocol names beginning with `_` are free for custom use, like other agent transport extension methods.
/// Protocol names that do not begin with `_` are reserved for the agent transport spec.
...
///
/// See protocol docs: [Agent Capabilities](https://agent-transport-protocol/protocol/initialization#agent-capabilities)
#[serde_as]
#[skip_serializing_none]
#[derive(Default, Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
...
/// capabilities.
///
/// See protocol docs: [Session Capabilities](https://agent-transport-protocol/protocol/initialization#session-capabilities)
#[serde_as]
#[skip_serializing_none]
#[derive(Default, Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
...
///
/// See protocol docs: [Prompt Capabilities](https://agent-transport-protocol/protocol/initialization#prompt-capabilities)
#[serde_as]
#[skip_serializing_none]
#[derive(Default, Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
...
pub struct McpCapabilities {
    ...
    /// This capability is not part of the spec yet, and may be removed or changed at any point.
    ///
    /// Agent supports [`McpServer::AgentTransport`].
    #[cfg(feature = "unstable_mcp_over_agent_transport")]
    ///
    /// Optional. Omitted or `null` both mean the agent does not advertise support.
    /// Supplying `{}` means the agent supports agent transport MCP server transports.
    #[serde_as(deserialize_as = "DefaultOnError")]
    #[schemars(extend("x-deserialize-default-on-error" = true))]
    #[serde(default, rename = "agent_transport")]
    pub agent_transport: Option<McpAgentTransportCapabilities>,
    /// The _meta property is reserved by agent transport to allow clients and agents to attach additional
    /// metadata to their interactions. Implementations MUST NOT make assumptions about values at
    /// these keys.
    ///
...
impl McpCapabilities {
    ...
    /// Supplying `{}` means the agent supports agent transport MCP server transports.
    #[must_use]
    pub fn agent_transport(
        mut self,
        agent_transport: impl IntoOption<McpAgentTransportCapabilities>,
    ) -> Self {
        self.agent_transport = agent_transport.into_option();
        self
    }
...
/// This capability is not part of the spec yet, and may be removed or changed at any point.
///
/// Capabilities for agent transport MCP server transports.
///
/// Supplying `{}` means the agent supports agent transport MCP server transports.
#[cfg(feature = "unstable_mcp_over_agent_transport")]
#[skip_serializing_none]
#[derive(Default, Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
#[non_exhaustive]
pub struct McpAgentTransportCapabilities {
    /// The _meta property is reserved by agent transport to allow clients and agents to attach additional
    /// metadata to their interactions. Implementations MUST NOT make assumptions about values at
    /// these keys.
    ///
...
#[cfg(feature = "unstable_mcp_over_agent_transport")]
impl McpAgentTransportCapabilities {
    /// Builds an empty [`McpAgentTransportCapabilities`]; use builder methods to advertise supported sub-capabilities.
    #[must_use]
    pub fn new() -> Self {
        Self::default()
    }
...
pub struct AgentMethodNames {
    ...
    /// Method for exchanging MCP-over-agent-transport messages.
    #[cfg(feature = "unstable_mcp_over_agent_transport")]
...
pub enum ClientRequest {
    ...
    ///
    /// See protocol docs: [Session Setup](https://agent-transport-protocol/protocol/session-setup)
    ...
    ///
    /// See protocol docs: [Prompt Lifecycle](https://agent-transport-protocol/protocol/prompt-lifecycle)
    ...
    /// This capability is not part of the spec yet, and may be removed or changed at any point.
    ///
    /// Exchanges an MCP-over-agent-transport message.
    #[cfg(feature = "unstable_mcp_over_agent_transport")]
...
pub enum AgentResponse {
    ...
    /// Successful result returned by an extension method outside the core agent transport method set.
    ExtMethodResponse(ExtResponse),
    /// Successful result returned by an MCP-over-agent-transport `mcp/message` request.
    #[cfg(feature = "unstable_mcp_over_agent_transport")]
...
pub enum ClientNotification {
    ...
    ///
    /// See protocol docs: [Cancellation](https://agent-transport-protocol/protocol/prompt-lifecycle#cancellation)
    ...
    /// This capability is not part of the spec yet, and may be removed or changed at any point.
    ///
    /// Sends an MCP-over-agent-transport notification.
    #[cfg(feature = "unstable_mcp_over_agent_transport")]
...
mod test_serialization {
    ...
    #[cfg(feature = "unstable_mcp_over_agent_transport")]
    #[test]
    fn test_mcp_capabilities_serialize_agent_transport_support_as_object() {
        let caps = McpCapabilities::new().agent_transport(McpAgentTransportCapabilities::new());
