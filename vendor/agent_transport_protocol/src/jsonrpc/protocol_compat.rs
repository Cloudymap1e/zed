...
mod imp {
    ...
    use agent_transport_protocol_schema::v2::{
        self,
    ...
    pub(crate) enum ProtocolMode {
        Disabled,
        AgentTransport(AgentTransportProtocolMode),
    }

    #[derive(Clone, Copy, Debug)]
    pub(crate) struct AgentTransportProtocolMode {
    ...
    impl ProtocolMode {
        ...
        pub(crate) fn v1_agent() -> Self {
            Self::AgentTransport(AgentTransportProtocolMode {
        ...
        pub(crate) fn merge(self, other: Self) -> Self {
            match (self, other) {
                ...
                (Self::AgentTransport(this), Self::AgentTransport(other)) => {
                    assert_eq!(
                        this.api, other.api,
                        "cannot merge agent transport builders with different API protocol versions; \
                         handler chains share a single API surface",
                    );
                    if this.latest_supported >= other.latest_supported {
                        Self::AgentTransport(this)
                    } else {
                        Self::AgentTransport(other)
                    }
                }
            }
        }
    ...
    #[derive(Clone, Debug)]
    pub(crate) struct ProtocolCompat {
        mode: Option<AgentTransportProtocolMode>,
    ...
    impl ProtocolCompat {
        pub(crate) fn new(mode: ProtocolMode) -> Self {
            Self {
                mode: match mode {
                    ...
                    ProtocolMode::AgentTransport(mode) => Some(mode),
                },
        ...
        fn incoming_initialize_request(
            &self,
            mode: AgentTransportProtocolMode,
            mut message: UntypedMessage,
        ) -> Result<UntypedMessage, crate::Error> {
        ...
        fn negotiate(&self, requested: ProtocolVersion) -> ProtocolVersionKind {
            let mode = self
                .mode
                .expect("agent transport protocol mode should be enabled while negotiating");
    ...
    fn invalid_initialize_protocol_version() -> crate::Error {
        crate::Error::invalid_params()
            .data("initialize.protocolVersion must be a valid agent transport protocol version")
    }
    ...
    fn unsupported_protocol_version(version: ProtocolVersion) -> crate::Error {
        crate::Error::invalid_request().data(format!(
            "unsupported agent transport protocol version {version}; this endpoint does not support that version"
        ))
    }
    ...
    fn required_protocol_version(
        ...
        crate::Error::invalid_request().data(format!(
            "required agent transport protocol version {} but peer negotiated {}; use a v1 client implementation for v1 agents",
    ...
    mod tests {
        ...
        fn incoming_initialize_response_requires_protocol_version() -> Result<(), crate::Error> {
            for value in [
                ...
                let error = compat
                    ...
                    .expect_err(
                        "initialize responses must declare an agent transport protocol version",
                    );
                let data = error
                    .data
                    .as_ref()
        ...
        #[test]
        #[should_panic(
            expected = "cannot merge agent transport builders with different API protocol versions"
        )]
