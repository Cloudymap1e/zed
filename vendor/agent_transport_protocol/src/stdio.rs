//! Stdio transport for connecting agent components via standard input/output.

use crate::external_agent_process::LineDirection;
use crate::{ByteStreams, ConnectTo, Role};
use std::sync::Arc;

/// A transport that connects to an agent-transport peer via standard input/output.
///
/// This is useful for building agents or proxies that communicate over stdio,
/// which is the standard transport for MCP and agent subprocess communication.
pub struct Stdio {
