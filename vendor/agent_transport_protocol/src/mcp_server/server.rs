//! MCP server attachment and routing for agent transport sessions.
...
/// An MCP server that can be attached to agent transport connections.
///
...
///
/// The `agent-transport-protocol-rmcp` crate provides builder APIs for MCP tools
...
pub struct McpServer<Counterpart: Role, Run = NullRun> {
    ...
    /// The agent transport identifier we assigned for this mcp server; always unique
    ...
    for McpServer<Counterpart, Run>
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("McpServer")
            ...
            .field("agent_transport_id", &self.agent_transport_id)
    ...
    Run: RunWithConnectionTo<Counterpart>,
{
    ...
    /// # See also
    ///
    /// See `agent-transport-protocol-rmcp` to construct MCP servers from Rust code
    /// with `rmcp`.
    pub fn new(c: impl McpServerConnect<Counterpart>, responder: Run) -> Self {
        McpServer {
            phantom: PhantomData,
            agent_transport_id: format!("agent-transport:{}", Uuid::new_v4()),
            connect: Arc::new(c),
            responder,
        }
    }
        ...
        Counterpart: HasPeer<Agent>,
    {
        ...
        (
            McpNewSessionHandler::new(agent_transport_id, connect),
            responder,
        )
    }
}
    ...
    Counterpart: HasPeer<Agent>,
{
    pub fn new(
        ...
        Self {
            active_session: McpActiveSession::new(agent_transport_id.clone(), connect.clone()),
            agent_transport_id,
            connect,
        }
    }
