...
/// Value attached to a given agent transport type on the `_meta` field.
///
/// The _meta property is reserved by agent transport to allow clients and agents to attach
...
/// Allows for sending an arbitrary request that is not part of the agent transport spec.
...
/// Allows for sending an arbitrary response to an [`ExtRequest`] that is not part of the agent transport spec.
/// Extension methods provide a way to add custom functionality while maintaining
/// protocol compatibility.
///
/// See protocol docs: [Extensibility](https://agent-transport-protocol/protocol/extensibility)
