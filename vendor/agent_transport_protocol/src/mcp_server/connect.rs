...
///
/// ```rust,ignore
/// use agent_transport_protocol::mcp_server::{McpServerConnect, McpConnectionTo};
/// use agent_transport_protocol::{DynConnectTo, role::Role};
///
/// struct MyMcpServer {
///     name: String,
/// }
...
pub trait McpServerConnect<Counterpart: Role>: Send + Sync + 'static {
    ...
    ///
    /// The [`McpConnectionTo`] provides access to the agent transport connection context and the
    /// server's agent transport URL.
