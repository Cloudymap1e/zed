...
description: Configure agent profiles for Zed-managed model selection, built-in tool availability, and MCP tool availability.
---

# Agent Profiles

Agent profiles control Zed-managed model and tool settings where an agent integration supports them. A profile can set a default model and choose which built-in tools and MCP tools are available.
...
If a tool is not available in the active profile, the agent cannot use it. If the tool is available and permission-gated, [Tool Permissions](./tool-permissions.md) still controls whether the tool call requires approval.

## Agent Path Boundaries {#agent-path-boundaries}

External Agents and Terminal Threads do not use Zed-managed profiles unless their integration explicitly supports similar behavior.
