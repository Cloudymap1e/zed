//! Utilities for connecting to external agents and proxies.
//!
//! This module provides [`ExternalAgentProcess`], a convenient wrapper around [`crate::schema::v1::McpServer`]
...
/// A component representing an external agent running in a separate process.
///
/// `ExternalAgentProcess` implements the [`ConnectTo`](`crate::ConnectTo`) trait for spawning and communicating with
...
/// ```
/// # use agent_transport_protocol::ExternalAgentProcess;
/// # use std::str::FromStr;
/// let agent = ExternalAgentProcess::from_str("python my_agent.py --verbose").unwrap();
/// ```
///
/// Parse from JSON:
/// ```
/// # use agent_transport_protocol::ExternalAgentProcess;
/// # use std::str::FromStr;
/// let agent = ExternalAgentProcess::from_str(r#"{"type": "stdio", "name": "my-agent", "command": "python", "args": ["my_agent.py"], "env": []}"#).unwrap();
/// ```
...
impl std::fmt::Debug for ExternalAgentProcess {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ExternalAgentProcess")
...
impl ExternalAgentProcess {
    /// Create a new `ExternalAgentProcess` from an [`crate::schema::v1::McpServer`] configuration.
    #[must_use]
    ...
    ///
    /// ```no_run
    /// # use agent_transport_protocol::{ExternalAgentProcess, LineDirection};
    /// # use std::str::FromStr;
    /// let agent = ExternalAgentProcess::from_str("python my_agent.py")
    ///     .unwrap()
    ...
    > {
        match &self.server {
            ...
            SchemaMcpServer::Http(_) => Err(crate::util::internal_error(
                "HTTP transport not yet supported by ExternalAgentProcess",
            )),
            SchemaMcpServer::Sse(_) => Err(crate::util::internal_error(
                "SSE transport not yet supported by ExternalAgentProcess",
            )),
...
/// Roles that an external agent executable can potentially serve.
pub trait ExternalAgentProcessCounterpartRole: Role {}

impl ExternalAgentProcessCounterpartRole for Client {}

impl ExternalAgentProcessCounterpartRole for Conductor {}

impl<Counterpart: ExternalAgentProcessCounterpartRole> crate::ConnectTo<Counterpart>
    for ExternalAgentProcess
{
    async fn connect_to(
        self,
...
impl ExternalAgentProcess {
    /// Create an `ExternalAgentProcess` from an iterator of command-line arguments.
    ///
    ...
    /// ```
    /// # use agent_transport_protocol::ExternalAgentProcess;
    /// let agent = ExternalAgentProcess::from_args([
    ///     "RUST_LOG=debug",
    ///     "cargo",
    ///     "run",
    ///     "-p",
...
impl FromStr for ExternalAgentProcess {
...
mod tests {
    ...
    fn test_parse_simple_command() {
        let agent = ExternalAgentProcess::from_str("python agent.py").unwrap();
        match agent.server {
            SchemaMcpServer::Stdio(stdio) => {
    ...
    fn test_parse_command_with_args() {
        let agent = ExternalAgentProcess::from_str("node server.js --port 8080 --verbose").unwrap();
        match agent.server {
            SchemaMcpServer::Stdio(stdio) => {
    ...
    fn test_parse_command_with_quotes() {
        let agent =
            ExternalAgentProcess::from_str(r#"python "my agent.py" --name "Test Agent""#).unwrap();
        match agent.server {
            SchemaMcpServer::Stdio(stdio) => {
                assert_eq!(stdio.name, "python");
                assert_eq!(stdio.command, PathBuf::from("python"));
    ...
    fn test_parse_json_stdio() {
        ...
        let agent = ExternalAgentProcess::from_str(json).unwrap();
        match agent.server {
            SchemaMcpServer::Stdio(stdio) => {
