//! Runtime-agnostic MCP server support for providing MCP tools over agent transport.
//!
//! This module provides the infrastructure for attaching MCP servers to agent sessions
//! connections without tying the core SDK to a particular MCP implementation or
//! async runtime.
//!
//! ## Building MCP servers with tools
//!
//! The `agent-transport-protocol-rmcp` crate provides the builder APIs for MCP
...
//!
//! ```rust,ignore
//! use agent_transport_protocol::mcp_server::{McpConnectionTo, McpServer, McpServerConnect};
//! use agent_transport_protocol::{DynConnectTo, NullRun, Role, role};
//!
