...
use agent_thread::{
    ...
    AgentSessionListResponse, AgentThread, PermissionOptions, SessionListUpdate,
    protocol as agent_protocol,
};
use anyhow::{Context as _, Result, anyhow, bail};
use chrono::{TimeZone as _, Utc};
use futures::FutureExt as _;
use gpui::{App, AppContext as _, AsyncApp, Entity, SharedString, Task, TaskExt as _, WeakEntity};
...
impl CodexNativeSessionList {
    ...
    fn send_info_update(
        &self,
        session_id: agent_protocol::SessionId,
        update: agent_protocol::SessionInfoUpdate,
    ) {
        self.updates_tx
...
pub struct CodexNativeConnection {
    ...
    client: CodexAppServerClient,
    sessions: Rc<RefCell<HashMap<agent_protocol::SessionId, CodexNativeSession>>>,
    pending_turn_starts:
        Rc<RefCell<HashMap<agent_protocol::SessionId, async_channel::Sender<String>>>>,
    pending_turns: Rc<RefCell<HashMap<String, async_channel::Sender<agent_protocol::StopReason>>>>,
    pending_thread_idle: Rc<
        RefCell<
            HashMap<agent_protocol::SessionId, async_channel::Sender<agent_protocol::StopReason>>,
        >,
    >,
    completed_turns: Rc<RefCell<HashMap<String, agent_protocol::StopReason>>>,
    auth_methods: Vec<agent_protocol::AuthMethod>,
...
pub async fn connect(
    ...
    let pending_thread_idle = Rc::new(RefCell::new(HashMap::new()));
    ...
    let dispatch_task = cx.spawn({
        ...
        let pending_thread_idle = pending_thread_idle.clone();
...
impl AgentConnection for CodexNativeConnection {
    ...
    fn new_session(
        ...
        cx.spawn(async move |cx| {
            ...
            let session_id = agent_protocol::SessionId::new(thread_id);
            let title = response
                .get("thread")
            ...
            let thread = cx.new(|cx| {
                AgentThread::new(
                    ...
                    action_log,
                    session_id.clone(),
                    watch::Receiver::constant(agent_protocol::PromptCapabilities::new()),
                    cx,
                )
            });
            ...
            thread
                .update(cx, |thread, cx| {
                    thread.handle_session_update(
                        agent_protocol::SessionUpdate::AvailableCommandsUpdate(
                            agent_protocol::AvailableCommandsUpdate::new(
                                native_available_commands(),
                            ),
                        ),
                        cx,
                    )
    ...
    fn auth_methods(&self) -> &[agent_protocol::AuthMethod] {
        &self.auth_methods
    }

    fn authenticate(
        &self,
        _method: agent_protocol::AuthMethodId,
        _cx: &mut App,
    ) -> Task<Result<()>> {
        Task::ready(Ok(()))
    }
    ...
    fn prompt(
        &self,
        params: agent_protocol::PromptRequest,
        cx: &mut App,
    ) -> Task<Result<agent_protocol::PromptResponse>> {
        let client = self.client.clone();
        let sessions = self.sessions.clone();
        let pending_turn_starts = self.pending_turn_starts.clone();
        let pending_turns = self.pending_turns.clone();
        let pending_thread_idle = self.pending_thread_idle.clone();
        ...
        cx.spawn(async move |cx| {
            ...
            let Some(command) = native_slash_command(&input)? else {
                ...
                .await;
                return Ok(agent_protocol::PromptResponse::new(stop_reason));
            };
    ...
    fn cancel(&self, session_id: &agent_protocol::SessionId, cx: &mut App) {
...
async fn handle_inbound_message(
    ...
    client: &CodexAppServerClient,
    sessions: &Rc<RefCell<HashMap<agent_protocol::SessionId, CodexNativeSession>>>,
    pending_turn_starts: &Rc<
        RefCell<HashMap<agent_protocol::SessionId, async_channel::Sender<String>>>,
    >,
    pending_turns: &Rc<RefCell<HashMap<String, async_channel::Sender<agent_protocol::StopReason>>>>,
    pending_thread_idle: &Rc<
        RefCell<
    ...
    match message {
        ...
        CodexInboundMessage::TransportClosed(reason) => {
            ...
            for sender in pending_turns.borrow_mut().drain().map(|(_, sender)| sender) {
                sender
                    .try_send(agent_protocol::StopReason::Cancelled)
                    .log_err();
            }
            for sender in pending_thread_idle
                .borrow_mut()
                .drain()
                .map(|(_, sender)| sender)
            {
...
async fn handle_server_notification(
    ...
    if method == "thread/status/changed" {
        complete_thread_idle(params, pending_thread_idle);
    }
    ...
    for (session_id, update) in updates {
        if let agent_protocol::SessionUpdate::SessionInfoUpdate(info_update) = &update {
...
async fn handle_approval_request(
    ...
    let session_id = agent_protocol::SessionId::new(thread_id.clone());
    let thread = sessions
        .borrow()
        .get(&session_id)
        .map(|session| session.thread.clone())
    ...
    let options = vec![
        agent_protocol::PermissionOption::new(
            allow_decision.to_owned(),
            "Allow",
            agent_protocol::PermissionOptionKind::AllowOnce,
        ),
        agent_protocol::PermissionOption::new(
            deny_decision.to_owned(),
            "Deny",
            agent_protocol::PermissionOptionKind::RejectOnce,
        ),
        agent_protocol::PermissionOption::new(
            "cancel",
            "Cancel",
    ...
    let tool_call = agent_protocol::ToolCallUpdate::new(
        agent_protocol::ToolCallId::new(item_id),
        agent_protocol::ToolCallUpdateFields::new()
            .kind(agent_protocol::ToolKind::Execute)
            .status(agent_protocol::ToolCallStatus::Pending)
            .title(title)
            .content(vec![agent_protocol::ToolCallContent::Content(
                agent_protocol::Content::new(agent_protocol::ContentBlock::Text(
                    agent_protocol::TextContent::new(content),
                )),
            )])
            .raw_input(params),
    );
    ...
    let decision = match outcome {
        agent_thread::RequestPermissionOutcome::Selected(selected)
            if selected.option_kind == agent_protocol::PermissionOptionKind::AllowOnce
                || selected.option_kind == agent_protocol::PermissionOptionKind::AllowAlways =>
        {
        ...
        agent_thread::RequestPermissionOutcome::Cancelled => "cancel".to_owned(),
    };
...
async fn handle_permissions_request(
    ...
    let tool_call = agent_protocol::ToolCallUpdate::new(
        agent_protocol::ToolCallId::new(item_id),
        agent_protocol::ToolCallUpdateFields::new()
            .kind(agent_protocol::ToolKind::Other)
            .status(agent_protocol::ToolCallStatus::Pending)
...
) -> Vec<(agent_protocol::SessionId, agent_protocol::SessionUpdate)> {
    match method {
        "item/agentMessage/delta" => {
            text_delta_update(params, agent_protocol::SessionUpdate::AgentMessageChunk)
        }
        "item/reasoning/textDelta" | "item/reasoning/summaryTextDelta" => {
            text_delta_update(params, agent_protocol::SessionUpdate::AgentThoughtChunk)
        }
...
fn text_delta_update(
    params: &Value,
    build: fn(agent_protocol::ContentChunk) -> agent_protocol::SessionUpdate,
) -> Vec<(agent_protocol::SessionId, agent_protocol::SessionUpdate)> {
    ...
    vec![(
        agent_protocol::SessionId::new(thread_id.to_owned()),
        build(agent_protocol::ContentChunk::new(
            agent_protocol::ContentBlock::Text(agent_protocol::TextContent::new(delta)),
        )),
    )]
}

fn plan_update(params: &Value) -> Vec<(agent_protocol::SessionId, agent_protocol::SessionUpdate)> {
    let Some(thread_id) = params.get("threadId").and_then(Value::as_str) else {
        return Vec::new();
    };
    let entries = params
        ...
        .filter_map(|entry| {
            ...
            let status = match entry.get("status").and_then(Value::as_str) {
                Some("completed") => agent_protocol::PlanEntryStatus::Completed,
                Some("inProgress") => agent_protocol::PlanEntryStatus::InProgress,
                _ => agent_protocol::PlanEntryStatus::Pending,
            };
            Some(agent_protocol::PlanEntry::new(
                step,
                agent_protocol::PlanEntryPriority::Medium,
                status,
            ))
        })
        .collect();
    ...
    vec![(
        agent_protocol::SessionId::new(thread_id.to_owned()),
        agent_protocol::SessionUpdate::Plan(agent_protocol::Plan::new(entries)),
    )]
}

fn thread_name_update(
    params: &Value,
) -> Vec<(agent_protocol::SessionId, agent_protocol::SessionUpdate)> {
    let Some(thread_id) = params.get("threadId").and_then(Value::as_str) else {
        return Vec::new();
    ...
    vec![(
        agent_protocol::SessionId::new(thread_id.to_owned()),
        agent_protocol::SessionUpdate::SessionInfoUpdate(
            agent_protocol::SessionInfoUpdate::new().title(title.to_owned()),
        ),
    )]
}

fn thread_goal_update(
    params: &Value,
) -> Vec<(agent_protocol::SessionId, agent_protocol::SessionUpdate)> {
    let Some(thread_id) = params.get("threadId").and_then(Value::as_str) else {
        return Vec::new();
...
fn text_message_update(
    ...
    vec![(
        agent_protocol::SessionId::new(thread_id.to_owned()),
        agent_protocol::SessionUpdate::AgentMessageChunk(agent_protocol::ContentChunk::new(
            agent_protocol::ContentBlock::Text(agent_protocol::TextContent::new(text)),
        )),
    )]
}
...
fn token_usage_update(
    ...
    vec![(
        agent_protocol::SessionId::new(thread_id.to_owned()),
        agent_protocol::SessionUpdate::UsageUpdate(agent_protocol::UsageUpdate::new(used, size)),
    )]
}
...
fn tool_output_delta_update(
    ...
    vec![(
        agent_protocol::SessionId::new(thread_id.to_owned()),
        agent_protocol::SessionUpdate::ToolCallUpdate(agent_protocol::ToolCallUpdate::new(
            item_id.to_owned(),
            agent_protocol::ToolCallUpdateFields::new().content(vec![
                agent_protocol::ToolCallContent::Content(agent_protocol::Content::new(
                    agent_protocol::ContentBlock::Text(agent_protocol::TextContent::new(
                        output.clone(),
                    )),
                )),
            ]),
...
fn file_change_patch_update(
    ...
    vec![(
        ...
        agent_protocol::SessionUpdate::ToolCallUpdate(agent_protocol::ToolCallUpdate::new(
            item_id.to_owned(),
            agent_protocol::ToolCallUpdateFields::new()
                .kind(agent_protocol::ToolKind::Edit)
                .content(vec![agent_protocol::ToolCallContent::Content(
                    agent_protocol::Content::new(agent_protocol::ContentBlock::Text(
                        agent_protocol::TextContent::new(summary),
                    )),
                )]),
        )),
    )]
...
fn item_lifecycle_update(
    ...
    let session_id = agent_protocol::SessionId::new(thread_id.to_owned());
    match item_type {
        "userMessage" if !completed => user_message_update(&session_id, item),
        "commandExecution" => tool_lifecycle_update(
            &session_id,
            item,
            agent_protocol::ToolKind::Execute,
            completed,
        ),
        "fileChange" => {
            tool_lifecycle_update(&session_id, item, agent_protocol::ToolKind::Edit, completed)
        }
        "mcpToolCall"
        | "dynamicToolCall"
        | "collabAgentToolCall"
        ...
        "exitedReviewMode" if completed => review_mode_result_update(&session_id, item),
        _ => Vec::new(),
    }
}
...
fn user_message_update(
    ...
    let blocks = item
        ...
        .map(|content| {
            (
                session_id.clone(),
                agent_protocol::SessionUpdate::UserMessageChunk(agent_protocol::ContentChunk::new(
                    content,
                )),
            )
        })
...
fn tool_lifecycle_update(
    ...
    if completed {
        let mut fields = agent_protocol::ToolCallUpdateFields::new()
            .kind(kind)
            .status(status);
        if let Some(content) = tool_item_content(item) {
            fields = fields.content(vec![agent_protocol::ToolCallContent::Content(
                agent_protocol::Content::new(agent_protocol::ContentBlock::Text(
                    agent_protocol::TextContent::new(content),
                )),
            )]);
        }
        let mut update = agent_protocol::ToolCallUpdate::new(item_id.to_owned(), fields);
        update.meta = collab_tool_call_meta(item);
        vec![(
            session_id.clone(),
            agent_protocol::SessionUpdate::ToolCallUpdate(update),
        )]
    } else {
        let mut tool_call = agent_protocol::ToolCall::new(item_id.to_owned(), tool_title(item))
            .kind(kind)
            .status(status)
        ...
        vec![(
            session_id.clone(),
            agent_protocol::SessionUpdate::ToolCall(tool_call),
        )]
    }
}

fn review_mode_result_update(
    session_id: &agent_protocol::SessionId,
    item: &Value,
) -> Vec<(agent_protocol::SessionId, agent_protocol::SessionUpdate)> {
    ...
    vec![(
        session_id.clone(),
        agent_protocol::SessionUpdate::AgentMessageChunk(agent_protocol::ContentChunk::new(
            agent_protocol::ContentBlock::Text(agent_protocol::TextContent::new(review.to_owned())),
        )),
    )]
}
    ...
    if completed {
        vec![(
            ...
            agent_protocol::SessionUpdate::ToolCallUpdate(agent_protocol::ToolCallUpdate::new(
                item_id,
                agent_protocol::ToolCallUpdateFields::new()
                    .status(agent_protocol::ToolCallStatus::Completed),
            )),
        )]
    } else {
        vec![(
            session_id,
            agent_protocol::SessionUpdate::ToolCall(
                agent_protocol::ToolCall::new(item_id, format!("Hook: {hook_name}"))
                    .kind(agent_protocol::ToolKind::Other)
                    .status(agent_protocol::ToolCallStatus::InProgress),
            ),
        )]
    }
}
...
fn complete_turn(
    ...
    let stop_reason = match turn.get("status").and_then(Value::as_str) {
        Some("interrupted") => agent_protocol::StopReason::Cancelled,
        _ => agent_protocol::StopReason::EndTurn,
    };
...
async fn wait_for_native_turn(
    ...
    turn_completed_rx
        .recv()
        .await
        .unwrap_or(agent_protocol::StopReason::Cancelled)
}

fn watch_next_thread_idle(
    session_id: &agent_protocol::SessionId,
    pending_thread_idle: &Rc<
        RefCell<
            HashMap<agent_protocol::SessionId, async_channel::Sender<agent_protocol::StopReason>>,
        >,
    >,
) -> async_channel::Receiver<agent_protocol::StopReason> {
    let (thread_idle_tx, thread_idle_rx) = async_channel::bounded(1);
    pending_thread_idle
        .borrow_mut()
        .insert(session_id.clone(), thread_idle_tx);
    thread_idle_rx
}

fn complete_thread_idle(
    params: &Value,
    pending_thread_idle: &Rc<
        RefCell<
            HashMap<agent_protocol::SessionId, async_channel::Sender<agent_protocol::StopReason>>,
    ...
    let stop_reason = match status_type {
        "idle" => agent_protocol::StopReason::EndTurn,
        "systemError" | "notLoaded" => agent_protocol::StopReason::Cancelled,
        _ => return,
    };
    let session_id = agent_protocol::SessionId::new(thread_id.to_owned());
    if let Some(sender) = pending_thread_idle.borrow_mut().remove(&session_id) {
        sender.try_send(stop_reason).log_err();
    }
}

async fn wait_for_thread_idle(
    thread_idle: async_channel::Receiver<agent_protocol::StopReason>,
) -> agent_protocol::StopReason {
    thread_idle
        .recv()
        .await
...
fn show_goal_status(
    session_id: &agent_protocol::SessionId,
    sessions: &HashMap<agent_protocol::SessionId, CodexNativeSession>,
    response: &Value,
    cx: &mut AsyncApp,
) -> Result<()> {
    ...
    let update =
        agent_protocol::SessionUpdate::AgentMessageChunk(agent_protocol::ContentChunk::new(
            agent_protocol::ContentBlock::Text(agent_protocol::TextContent::new(message)),
        ));
    thread
        .update(cx, |thread, cx| thread.handle_session_update(update, cx))?
        .map_err(anyhow::Error::from)
...
fn prompt_blocks_to_codex_input(blocks: Vec<agent_protocol::ContentBlock>) -> Result<Vec<Value>> {
    let mut input = Vec::new();
    for block in blocks {
        match block {
            agent_protocol::ContentBlock::Text(text) => input.push(json!({
                "type": "text",
                "text": text.text,
            })),
            agent_protocol::ContentBlock::Resource(resource) => input.push(json!({
                "type": "text",
                "text": serde_json::to_string(&resource).context("failed to serialize prompt resource")?,
            })),
            agent_protocol::ContentBlock::ResourceLink(resource) => input.push(json!({
                "type": "text",
                "text": serde_json::to_string(&resource).context("failed to serialize prompt resource link")?,
            })),
            agent_protocol::ContentBlock::Image(image) => input.push(json!({
                "type": "text",
                "text": format!("[Image content omitted by native Codex Zed adapter: {:?}]", image),
            })),
            agent_protocol::ContentBlock::Audio(audio) => input.push(json!({
                "type": "text",
...
async fn run_native_slash_command(
    ...
    cx: &mut AsyncApp,
) -> Result<agent_protocol::PromptResponse> {
    match command {
        NativeSlashCommand::Review { target } => {
            let thread_idle = watch_next_thread_idle(session_id, pending_thread_idle);
                ...
                .await
            {
                ...
                Err(error) => {
                    pending_thread_idle.borrow_mut().remove(session_id);
                    return Err(error).context("failed to start native Codex review");
                }
            };
            let turn_id = response
                .get("turn")
            ...
            set_active_turn(session_id, turn_id, sessions);
            let stop_reason = wait_for_thread_idle(thread_idle).await;
            clear_active_turn(session_id, sessions);
            return Ok(agent_protocol::PromptResponse::new(stop_reason));
        }
        ...
        NativeSlashCommand::Goal(command) => {
            let params = match command {
                GoalCommand::Status => {
                    ...
                    return Ok(agent_protocol::PromptResponse::new(
                        agent_protocol::StopReason::EndTurn,
                    ));
                }
                GoalCommand::Clear => {
                    client
    ...
    Ok(agent_protocol::PromptResponse::new(
        agent_protocol::StopReason::EndTurn,
    ))
}

fn user_input_to_content_block(input: &Value) -> Option<agent_protocol::ContentBlock> {
    match input.get("type").and_then(Value::as_str) {
        Some("text") => input
            .get("text")
            .and_then(Value::as_str)
            .map(|text| agent_protocol::ContentBlock::Text(agent_protocol::TextContent::new(text))),
        Some("localImage") => input.get("path").and_then(Value::as_str).map(|path| {
            agent_protocol::ContentBlock::Text(agent_protocol::TextContent::new(format!(
                "[Local image: {path}]"
            )))
        }),
        Some("image") => input.get("url").and_then(Value::as_str).map(|url| {
            agent_protocol::ContentBlock::Text(agent_protocol::TextContent::new(format!(
                "[Image: {url}]"
            )))
        }),
...
fn collab_tool_call_meta(item: &Value) -> Option<agent_protocol::Meta> {
...
fn tool_status_from_item(item: &Value, completed: bool) -> agent_protocol::ToolCallStatus {
    match item.get("status").and_then(Value::as_str) {
        Some("completed") => agent_protocol::ToolCallStatus::Completed,
        Some("failed") | Some("declined") => agent_protocol::ToolCallStatus::Failed,
        _ if completed => agent_protocol::ToolCallStatus::Completed,
        _ => agent_protocol::ToolCallStatus::InProgress,
    }
}

fn tool_kind_for_item_type(item_type: &str) -> agent_protocol::ToolKind {
    match item_type {
        "webSearch" => agent_protocol::ToolKind::Fetch,
        "fileChange" => agent_protocol::ToolKind::Edit,
        "commandExecution" => agent_protocol::ToolKind::Execute,
        _ => agent_protocol::ToolKind::Other,
    }
}
...
fn turn_key(session_id: &agent_protocol::SessionId, turn_id: &str) -> String {
...
fn native_available_commands() -> Vec<agent_protocol::AvailableCommand> {
    vec![
        agent_protocol::AvailableCommand::new(
            "review",
            "Review my current changes and find issues",
        )
        .input(agent_protocol::AvailableCommandInput::Unstructured(
            agent_protocol::UnstructuredCommandInput::new("optional custom review instructions"),
        )),
        agent_protocol::AvailableCommand::new(
            "init",
        ...
        agent_protocol::AvailableCommand::new(
            "goal",
            "Set, show, pause, resume, or clear the thread goal",
        )
        .input(agent_protocol::AvailableCommandInput::Unstructured(
            agent_protocol::UnstructuredCommandInput::new("objective | pause | resume | clear"),
        )),
        agent_protocol::AvailableCommand::new("resume", "Resume a native Codex thread"),
        agent_protocol::AvailableCommand::new("fork", "Fork a native Codex thread"),
        agent_protocol::AvailableCommand::new("history", "Show native Codex thread history"),
        agent_protocol::AvailableCommand::new("model", "Show or change the Codex model"),
        agent_protocol::AvailableCommand::new("config", "Show or change Codex configuration"),
        agent_protocol::AvailableCommand::new("skills", "List Codex skills"),
        agent_protocol::AvailableCommand::new("plugins", "List Codex plugins"),
        agent_protocol::AvailableCommand::new("hooks", "List Codex hooks"),
        agent_protocol::AvailableCommand::new("mcp", "List Codex MCP servers"),
    ]
}

fn agent_session_info_from_thread_value(value: &Value) -> Option<AgentSessionInfo> {
    let id = value.get("id")?.as_str()?;
    let mut info = AgentSessionInfo::new(agent_protocol::SessionId::new(id.to_owned()));
    info.title = value
        .get("name")
        .and_then(Value::as_str)
        .map(SharedString::from);
...
mod tests {
    ...
    #[test]
    fn maps_agent_delta_to_protocol_update() {
        ...
        match &updates[0].1 {
            agent_protocol::SessionUpdate::AgentMessageChunk(chunk) => match &chunk.content {
                agent_protocol::ContentBlock::Text(text) => assert_eq!(text.text, "hello"),
    ...
    #[test]
    fn maps_review_completion_to_visible_message() {
        let mut tool_outputs = HashMap::new();
        let updates = session_updates_from_notification(
            "item/completed",
            &json!({
        ...
        match &updates[0].1 {
            agent_protocol::SessionUpdate::AgentMessageChunk(chunk) => match &chunk.content {
                agent_protocol::ContentBlock::Text(text) => assert_eq!(text.text, "No findings."),
                other => panic!("unexpected content block: {other:?}"),
            },
            other => panic!("unexpected update: {other:?}"),
        }
    ...
    #[test]
    fn completes_review_waiter_on_thread_idle() {
        let pending_thread_idle = Rc::new(RefCell::new(HashMap::new()));
        let session_id = agent_protocol::SessionId::new("thread-1");
        let thread_idle = watch_next_thread_idle(&session_id, &pending_thread_idle);
        complete_thread_idle(
            &json!({
                "threadId": "thread-1",
                "status": {
                    "type": "active",
        ...
        assert!(thread_idle.try_recv().is_err());
        ...
        assert_eq!(
            thread_idle.try_recv().ok(),
            Some(agent_protocol::StopReason::EndTurn)
        );
        assert!(!pending_thread_idle.borrow().contains_key(&session_id));
    }
    ...
    fn maps_token_usage_to_zed_meta() {
        ...
        assert_eq!(updates.len(), 1);
        match &updates[0].1 {
            agent_protocol::SessionUpdate::UsageUpdate(update) => {
    ...
    fn maps_goal_update_to_visible_message() {
        ...
        match &updates[0].1 {
            agent_protocol::SessionUpdate::AgentMessageChunk(chunk) => match &chunk.content {
                agent_protocol::ContentBlock::Text(text) => {
